...
def main():
    ...
    global PROTEKTOR3000_ACTIVATED
...
class PROTEKTOR3000:
    ...
    """ class to encrypt and decrypt data using Fernet
    (learned from https://www.geeksforgeeks.org/encrypt-and-decrypt-files-using-python/) """
        ...
        for row in reader:
            if consult_id.strip() == row['ID'].strip():
                ...
                summary.insert(tk.END, f"""{row['Appointment Date']} appointment summary:\n
                        Patient: {row['Name']}, Age: {age},
                        IMC: {round(float(row['Weight']) / ((float(row['Height']) / 100) ** 2), 2)}\n
                        Complain: {row['Current Complain']},
                        Comorbidities: {row['Comorbidities']}\n
                        Current medications: {row['Medications']}\n
                        Allergies: {row['Allergies']},
                        BP:{row['Systolic Blood Pressure']}/{row['Diastolic Blood Pressure']}mmHg
                        HR: {row['Heart Rate']} bpm, RR: {row['Respiratory Rate']} bpm \n""")
        ...
        with open(f"{full_record}.csv", 'w', newline='', encoding='utf-8') as csvfile:
            ...
            for row in reader:
                ...
                if full_record.strip() == row['Name'].strip():
                    ...
                    top_row_fields = [key for key in list(filtered.keys())[:5]] +\
                        [key for key in vital_signs.keys()]
